...
def perform_shuffles_helper(args):
    ...
    vecpars["n_pre_switch_lvl"]["time_win"] = [-3, 3]  # select all
    vecpars["choice"] = {}
    vecpars["choice"]["time_win"] = [-3, 3]  # select all
    plotflag = False
    ...
    vecpars = {}
    vecpars["reward"] = {}
    vecpars["reward"]["time_win"] = [-3, 3]  # select all
    vecpars["choice"] = {}
    vecpars["choice"]["time_win"] = [-3, 3]  # select all
...
def main(args):
    subject, event, tmin, tmax = args
    postfix = f"_{tmin}_{tmax}"
    datadir = findrootdir()
    time_window = [-3, 3]
    load_event = event
    ...
    for session_id in session_ids:
        ...
        with open(
            f"{datadir}/stats_paper/{subject}_{event}_act_obs_dimensions_{date}{postfix}.json",
            "w",
        ) as f:
            json.dump(results, f)
    n_shuffles = 100
    ...
    with open(
        f"{datadir}/stats_paper/{result_name}{postfix}.json",
        "w",
    ) as f:
        json.dump(results, f)
    ...
    for event in ["fdbk"]:
        for animal in ["O", "L"]:
            for tmin, tmax in [
                ...
                print(f"Processing {animal} {event} from {tmin} to {tmax}")
                main((animal, event, tmin, tmax))
